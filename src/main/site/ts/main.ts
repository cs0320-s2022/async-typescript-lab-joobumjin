...
const sun = document.querySelector("#sun") as HTMLOptionElement
const moon = document.querySelector("#moon") as HTMLOptionElement
const rising = document.querySelector("#rising") as HTMLOptionElement
...
function postAndUpdate(): void {
  ...
  fetch('http://127.0.0.1:4567/results', {
    method: 'POST',
    body: JSON.stringify(
        postParameters
    ),
...
function updateSuggestions(matches: string[]): void {
  ...
  for (const match of matches) {
    // @ts-ignore
    matchList.innerHTML += '<li tabindex="0">' +
        match +
        '</li>';
  }
}
...
const myValues:MatchesRequestData = {
  sun: "Capricorn",
  moon: "Capricorn",
  rising: "Aquarius"
}
document.addEventListener("keyup", event => {
  if (event.key === "Enter") {
    updateValues(myValues.sun, myValues.moon, myValues.rising).then(r => postAndUpdate());
  }
})
